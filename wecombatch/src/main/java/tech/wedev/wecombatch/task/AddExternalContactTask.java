...
@Slf4j
@Component("AddExternalContactTask")
public class AddExternalContactTask implements IAsynTask {
    ...
    public AsynTaskErrInfo executeTask(AsynTaskBean task) {
        ...
        log.info(">>>>>>>>>>>>>>>>>>>>>>>企微好友添加事件回调异步任务开始执行>>>>>>>>>>>>>>>>>>>>>>>");
        try {
            errInfo = doBusiness(task);
        } catch (Exception e) {
            log.error("企微好友添加事件回调异步任务执行时异常，异常信息: " + e.getMessage());
            errInfo.setSuccFlag(false);
            errInfo.setErrMsg("企微好友添加事件回调异步任务执行时异常，异常信息: " + e.getMessage() + ">>>" + LocalDateTime.now());
            return errInfo;
        }
        log.info(">>>>>>>>>>>>>>>>>>>>>>>企微好友添加事件回调异步任务结束>>>>>>>>>>>>>>>>>>>>>>>");
        return errInfo;
    }
    ...
    private AsynTaskErrInfo doBusiness(AsynTaskBean task) {
        ...
        log.info("添加客户事件回调###入参: " + JSONObject.toJSONString(postMap));
    ...
    public QywxCustRel getQywxData(String qywxMrgId, String qywxCustId, String corpId) throws Exception {
        ...
        if (!"0".equals(errcode)) {
            log.error("添加客户事件回调###调用企微API获取客户企微侧数据失败: " + errcode);
            return null;
        } else {
            ...
            qywxCustRelService.insertBatch(qywxCustRelList);
            return qywxCustRel;
        }
    }
}
